...
//this structure will track processes waiting for the I/O
struct WaitingProcess {
    int PID;
    unsigned int io_start_time;
    unsigned int io_end_time;

    WaitingProcess(int pid, unsigned int start, unsigned int duration) : PID(pid), io_start_time(start), io_end_time(start + duration){}
};
