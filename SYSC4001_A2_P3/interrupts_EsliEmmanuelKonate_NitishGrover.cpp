...
std::tuple<std::string, std::string, int> simulate_trace(std::vector<std::string> trace_file, int time, std::vector<std::string> vectors, std::vector<int> delays, std::vector<external_file> external_files, PCB current, std::vector<PCB> wait_queue) {
    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        if(activity == "CPU") { //As per Assignment 1
            ...
            //now I will create a child PCB
            static int next_pid = 1;
            PCB child(next_pid++, current.PID, current.program_name, current.size, -1);

            //now I allocate memory for the child
            if(!allocate_memory(&child)){
                std::cerr <<"The Memory allocation has failed for the child!" << std::endl;
                continue;
            }
            execution += std::to_string(current_time) + ", 0, the scheduler called\n";
            execution += std::to_string(current_time) + ", 1, IRET\n";
            current_time += 1;
            ...
            //now the child will become  current and the parent will wait in queue
            PCB child_current = child;
            wait_queue.push_back(current);


            // Now we log the System Status
            system_status += "time: " + std::to_string(current_time) + "; current trace: FORK, " + std::to_string(duration_intr) + "\n";
            system_status += print_PCB(child_current, wait_queue);
            system_status += "\n";
            
            //now we execute the child

            auto [child_exec, child_status, child_time] = simulate_trace(child_trace, current_time, vectors, delays, external_files, child_current, wait_queue);

            execution += child_exec;
            system_status += child_status;
            current_time = child_time;

            //the child has finished so memory is freed and it is removed from the wait queue
            free_memory(&child_current);

            //restore the parent as current now
            ...
            execution += std::to_string(current_time) + ", " + std::to_string(duration_intr) +  ", ISR execution\n";
            current_time += duration_intr;

            //get the program size from the external files
            ...
            current_time += program_size;

            //the loader is 15 ms per MB
            int load_time = program_size * 15;
            execution += std::to_string(current_time) + ", " + std::to_string(load_time) + ", loading program into the memory\n";
            current_time += load_time;

            execution += std::to_string(current_time) + ", 3, is marking the part as occupied\n";
            current_time += 3;
            ...
            //Now we update the current PCB with the new program
            current.program_name = program_name;
            current.size = program_size;
            current.partition_number = -1;

            //again we allocate memory
            if(!allocate_memory(&current)){
                std::cerr << "Error! The memory allocation has failed for exec!" << std::endl;
                continue;
            }
            ...
            //again we log the status after EXEC
            ...
            //here we will execute the trace of the program
            ...
            //I will add this later on in the report : 
            //It is important because exec() will replace the current program.
            //The old program will not longer exists, so we cannotcontinue executing the rest of the 
            //trace file. The new program's trace will be run instead.
